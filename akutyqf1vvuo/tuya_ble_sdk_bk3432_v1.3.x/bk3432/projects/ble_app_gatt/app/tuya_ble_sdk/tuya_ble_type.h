...
typedef enum {
    ...
    TUYA_BLE_EVT_AUTO_RECOVERY_FROM_RESETTING_TIMEOUT,
} tuya_ble_evt_t;
...
typedef enum {
    ...
    TUYA_BLE_CB_EVT_AUTO_RECOVERY_FROM_RESETTING_TIMEOUT,
    TUYA_BLE_CB_EVT_UNBIND_RESET_RESPONSE,               // Notify the application of the result of the local reset
    TUYA_BLE_CB_EVT_UNBIND_RESET_WITH_AR_RESPONSE, 
} tuya_ble_cb_evt_t;
...
typedef struct {
    tuya_ble_reset_type_t type;
    uint8_t status;     //0-succeed,1-failed.
} tuya_ble_unbind_reset_with_ar_response_data_t;  //auto recovery
...
typedef struct {
	...
	union {
        ...
        tuya_ble_unbind_reset_with_ar_response_data_t reset_with_ar_response_data;
	};
